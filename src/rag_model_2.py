...
class RAGModel:
    def __init__(self, api_key, model_name):
        ...
        self.jacolbert = RAGPretrainedModel.from_pretrained("bclavie/JaColBERTv2")

    def prepare_documents(self, search_data: List[Dict]) -> None:
        """
        Prepare and encode the search data using JaColBERT
        ...
        # Clear any existing encoded documents
        try:
            self.jacolbert.clear_encoded_docs()
            print("JaColBERT encode successed")
        except AttributeError:
            print("JaCoBERT encode failed")
            pass
        
        # Encode documents using JaColBERT
        self.jacolbert.encode(self.documents)
    ...
    def get_relevant_context(self, query: str, yes_with_evidence_count: int = 6, yes_without_evidence_count: int = 2, no_promise_count: int = 2) -> List[Dict]:
        """
        JaColBERTを使用して文書を検索し、promise_statusとevidence_statusの特定の比率を維持して文書を取得します。

        Args:
            query (str): 入力クエリ
            yes_with_evidence_count (int): promise_status "Yes" かつ evidence_status "Yes" の文書の取得数
            yes_without_evidence_count (int): promise_status "Yes" かつ evidence_status "No" の文書の取得数
            no_promise_count (int): promise_status "No" の文書の取得数

        Returns:
            List[Dict]: 指定された状態の分布を持つ関連文書のリスト
        """
        try:
            total_needed = yes_with_evidence_count + yes_without_evidence_count + no_promise_count
            search_results = self.jacolbert.search_encoded_docs(
                query=query,
                k=total_needed
            )

            # 検索結果を元のデータと対応付け、スコアを保持
            ranked_docs = []
            for result in search_results:
                doc_index = self.documents.index(result['content'])
                ranked_docs.append((
                    doc_index,
                    result['score'],
                    {
                        'promise_status': self.search_data[doc_index].get('promise_status', 'No'),
                        'evidence_status': self.search_data[doc_index].get('evidence_status', 'N/A')
                    }
                ))

            # ステータスの組み合わせで文書を分類
            yes_with_evidence = [
                (i, score) for i, score, status in ranked_docs
    ...
    def analyze_paragraph(self, paragraph: str) -> Dict[str, str]:
        ...
        Task Steps:
        1. Read the examples in <extraction/classification examples> carefully and learn the features of what content is considered to be promise or evidence, and the classification features of "verification_timeline" and "evidence_quality".
