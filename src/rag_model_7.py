## rag_model_3, rag_model_5の統合

# 抽出をgptでやった後、分類をgptでやるように、2段階にした（検索方法はrag_modelと同じくE5による密検索）
# 抽出部分・分類部分のどちらにも、"explain"付きのデータを検索対象とした場合（2段階目の分類では"explain"無し）（プロンプトもそれに合わせて微修正）（search_data_path: "data/processed/Japanese_train_experiment_explanained.json”とする！）
