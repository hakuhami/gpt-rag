## rag_model_7を拡張：RAG無しで、公約、検証時期、根拠、根拠の性質の4段階の分類を行うモデル（検索方法はrag_modelと同じくE5による密検索）
...
class RAGModel:
    ...
    def classify_step1_promise(self, data: str) -> Dict[str, str]:
        ...
        prompt = f"""
        You are an expert in extracting and classifying promise from corporate texts related to ESG.
        Extract and classify the promise from the provided test data, and output each content to the corresponding label in the json format.
        ...
        "promise_string" should be extracted verbatim from the original text. If there is no corresponding text (when promise_status is No), output a blank.
        ...
        response = self.client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are an expert in extracting ESG-related promise from corporate reports that describe ESG matters."},
                {"role": "user", "content": prompt}
            ],
            functions=[{
                "name": "analyze_esg_paragraph",
                "description": "Analyze an ESG-related paragraph and extract promise information",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "data": {"type": "string"},
                    ...
                    "required": ["data", "promise_status", "promise_string"]
                }
            }],
    ...
    def classify_step3_evidence(self, data: str, promise_string: str) -> Dict[str, str]:
        ...
        Task Steps:
        1. Carefully read the contents of the test data.
        2. Classification task (About "evidence_status"):
